...
bot = Bot(token=TOKEN)
dp = Dispatcher()
# Use /data directory if it exists (for Railway Volumes), otherwise use local file
DB_PATH = "/data/store.db" if os.path.exists("/data") else "store.db"
...
def main_menu_kb():
    return InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text="💳 شحن الرصيد (يدوي)", callback_data="topup_manual")],
...
async def list_categories():
    async with aiosqlite.connect(DB_PATH) as db:
        cur = await db.execute("SELECT category, COUNT(id) FROM stock WHERE IFNULL(is_sold,0)=0 GROUP BY category ORDER BY category")
        return await cur.fetchall()
...
async def find_item_with_mode(category: str, mode: str):
    async with aiosqlite.connect(DB_PATH) as db:
        ...
        items = await cur.fetchall()
    for r in items:
        chosen = r[14] if len(r) > 14 else None
        rem = max((r[{"personal": 6, "shared": 9, "laptop": 12}[mode]] or 0) - (r[{"personal": 7, "shared": 10, "laptop": 13}[mode]] or 0), 0)
        pr = r[{"personal": 5, "shared": 8, "laptop": 11}[mode]]
...
async def cb_balance(c: CallbackQuery):
    ...
    await c.message.edit_text(f"رصيدك الحالي: {bal:g} ج.м", reply_markup=main_menu_kb())
@dp.callback_query(F.data == "topup_manual")
async def cb_topup_manual(c: CallbackQuery):
    await c.message.edit_text("لشحن رصيدك، يرجى التواصل مع الأدمن وإرسال إثبات الدفع.", reply_markup=InlineKeyboardMarkup(inline_keyboard=[[InlineKeyboardButton(text="🔙 رجوع", callback_data="back_home")]]))
...
async def cb_catalog(c: CallbackQuery):
    ...
    if not rows: await c.message.edit_text("لا يوجد مخزونات حاليًا.", reply_markup=main_menu_kb()); return
...
async def cb_pick_category(c: CallbackQuery):
    ...
    for m in ["personal","shared","laptop"]:
        if m in modes_info:
            mi = modes_info[m]
            rows.append([InlineKeyboardButton(text=f"{name[m]} — من {mi['min_price']:g} ج.м ({mi['count']} عنصر)", callback_data=f"mode::{category}::{m}")])
    rows.append([InlineKeyboardButton(text="🔙 رجوع", callback_data="catalog")])
    await c.message.edit_text(f"الفئة: {category}\nاختر النوع:", reply_markup=InlineKeyboardMarkup(inline_keyboard=rows))
...
async def cb_pick_mode(c: CallbackQuery):
    ...
    price = item[{"personal": 5, "shared": 8, "laptop": 11}[mode]]
    await c.message.edit_text(f"الفئة: {category}\nالنوع: {mode}\nالسعر: {price:g} ج.м\nاضغط شراء لإتمام العملية.", reply_markup=InlineKeyboardMarkup(inline_keyboard=[[InlineKeyboardButton(text="✅ شراء الآن", callback_data=f"buy::{category}::{mode}")],[InlineKeyboardButton(text="🔙 رجوع", callback_data=f"cat::{category}")]]))
...
async def import_file_handler(m: Message):
    st = getattr(dp, "workflow_state", {}); w_m = st.get("awaiting_importm"); w_s = st.get("awaiting_import")
    if not (w_m or w_s) or not is_admin(m.from_user.id): return
    if (w_m and w_m.get("admin") != m.from_user.id) or (w_s and w_s.get("admin") != m.from_user.id): return
