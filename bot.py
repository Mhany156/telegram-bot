...
# Kashier
KASHIER_API_KEY = os.getenv("KASHIER_API_KEY", "")          # Payment API Key (للتوقيع)
KASHIER_MERCHANT_ID = os.getenv("KASHIER_MERCHANT_ID", "")  # للمرجع فقط إن احتجناه
KASHIER_SECRET = os.getenv("KASHIER_SECRET", "")            # للاستخدام المستقبلي مع REST إن لزم

# Payment Pages
PP_PERSONAL = os.getenv("KASHIER_PP_PERSONAL", "")  # CapCut 1 Month (personal)
PP_SHARED  = os.getenv("KASHIER_PP_SHARED", "")     # CapCut 1 Month (shared)
PP_LAPTOP  = os.getenv("KASHIER_PP_LAPTOP", "")     # Laptop
...
# ==================== نموذج مخزون بسيط (بدون DB) ====================
# لو عندك DB بالفعل بدّل الدوال الأربعة تحت (find_item_with_mode / price_for_mode / increment_sale_and_finalize / log_sale)
STOCK = {
...
async def cb_category(c: CallbackQuery):
    ...
    for mode_key, items in modes.items():
        if items:
            # عرض اسم ودّي للمستخدم جنب مفتاح المود
...
async def cb_pick_mode(c: CallbackQuery):
    ...
    await c.message.edit_text(
        f"الفئة: {escape(category)}\nالنوع: {escape({'personal':'فردي','shared':'مشترك','laptop':'لابتوب'}.get(mode, mode))}\nالسعر: {price:g} ج.م\nاضغط الدفع لإتمام العملية.",
        reply_markup=kb
    )
...
def _kashier_verify_signature(raw_body: bytes, received_sig: str) -> bool:
    """
    التوقيع: HMAC-SHA256 على Body باستخدام Payment API Key
    """
...
def kashier_callback():
    try:
        ...
        async def finalize():
            ...
            msg = f"✅ تم الدفع.\n\n📦 <b>{escape(category)} — {escape({'personal':'فردي','shared':'مشترك','laptop':'لابتوب'}.get(mode, mode))}</b>\n📩 <b>بياناتك:</b>\n<code>{credential}</code>"
            if instructions:
                msg += f"\n\n{instructions}"
